...
let chat_state = (sessionStorage.getItem('ai_state') && JSON.parse(sessionStorage.getItem('ai_state'))[0].user === '') 
? JSON.parse(sessionStorage.getItem('ai_state')) 
: []
...
const continue_chat_url = (url) => {
    sessionStorage.setItem('cont_url', url)
}

async function get_chat_history() {
    const api_call = await fetch('https://reflexshop.app.n8n.cloud/webhook-test/api/get-chat-history', {
        method: 'GET',
...
async function callAPI() {
    ...
    if(chat_state.length === 0) {
        ...
        api = await fetch("https://reflexshop.app.n8n.cloud/webhook-test/api/chat", {
            method: "POST",
            headers: {"Content-Type": "application/json"},
        ...
        api = await fetch(sessionStorage.getItem('cont_url'), {
            method: "POST",
            headers: {"Content-Type": "application/json"},
            body: JSON.stringify({q: user_input, state: chat_state, http_path: 'get'}),
        }).catch((e) => {chat_div.innerHTML = e.message})
    ...
    make_message.id = 'agent_response'

    continue_chat_url(response.url)
